...
This repo targets .NET 11. Prefer Docker because the host may not have the .NET SDK installed. On Apple Silicon, build and run as `linux/amd64`; the published app includes `IBM.Data.Db2.dll`, which is not compatible with an ARM64 process.
