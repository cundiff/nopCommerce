...
echo "=== COLD BASELINE ===" && echo ""
claude -p "$TEST_Q" | tee demo-outputs/cc_baseline.txt
echo ""
...
claude -p "${WARMUPS[0]}" > demo-outputs/cc_warmup_01.txt
echo "1/12 done"

for i in "${!WARMUPS[@]}"; do
  if [ "$i" -gt 0 ]; then
    PADDED=$(printf "%02d" $((i+1)))
    claude -c -p "${WARMUPS[$i]}" > "demo-outputs/cc_warmup_${PADDED}.txt"
    echo "$((i+1))/12 done"
    sleep 1
  fi
done

echo "" && echo "=== POST-WARMUP ===" && echo ""
claude -c -p "$TEST_Q" | tee demo-outputs/cc_result.txt

echo "" && echo "=== COMPARISON ===" && echo ""
echo "BASELINE:" && cat demo-outputs/cc_baseline.txt
echo "" && echo "POST-WARMUP:" && cat demo-outputs/cc_result.txt
